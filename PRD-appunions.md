...
先创建应用资料，再到详情页选择支持的平台：

| 字段 | 说明 |
| --- | --- |
| 名称 | 展示给终端用户 |
| 图标 | 用于列表卡片 |
| 描述 | 建议不超过 30 个汉字，列表卡片上展示 |
| 分类 | 大分类 + 小分类。点输入框提示已有项，也可输入新的；运营可在后台编辑目录 |

创建后再勾选支持的端，并按端填写包名（客户端凭包名打开对应应用商店，不提供商店 URL、也不提供自定义跳转）：

| 端 | 包名 |
| --- | --- |
| Android | applicationId，如 `com.company.app` |
| iOS | Bundle ID |
| 鸿蒙 | bundleName |

一个 App 可同时支持多个端，同一把 API Key。开放接口必须带当前端的 `platform`。
...
- 请求必须带上该 App 的 key。服务端校验 key 对应已通过的宿主，且该宿主已配置请求中的 `platform`。
...
第一期只提供四类接口。鉴权：请求头携带 `api_key`。所有列表接口必须带当前端 `platform`，只返回已配置该端的 App，并排除自己。

**1. 随机推荐**

- `GET /v1/apps/recommend`
- 必填 query `platform`。从该端推荐池中等权随机抽取，最多 10 条；池里不足 10 条则有多少返回多少。
...
- `GET /v1/apps`
- 必填 query `platform`。返回该端所有「已通过且未暂停」且已填写该端包名的 App，含暂未进入推荐池的（例如观察期未达标）。这样「查看全部」仍然完整。
...
`id`、名称、图标 URL、描述、分类、系统、包名。客户端用包名打开对应应用商店，不返回商店 URL 或自定义跳转。
...
1. 接入步骤：注册 → 建 App → 填各端包名 → 等审核 → 拿 key → 带 platform 拉推荐 → 展示 → 报曝光 / 点击。
2. 鉴权方式和四个接口的字段说明。
3. 曝光、点击的上报时机（可视区、点击后再跳转）。
4. 三端打开方式：用返回的 `package_name` 打开对应应用商店（Android `market://details?id=`，iOS Bundle ID，鸿蒙 bundleName）。不提供商店 URL 或自定义跳转。
5. 错误码和处理建议。
...
| A4 | 各端用包名足以打开对应应用商店 | 某端打开失败多，需要补该端的打开约定 |
...
- **审核质量**：违规 App 进池会伤害所有接入者的用户。宁可不快，也不要放松包名真实性和内容底线。
- **三端差异**：国内 Android 多商店、iOS 用 Bundle ID 打开商店的方式可能比 Android 更麻烦。文档必须按端写清，审核时按端验收。
