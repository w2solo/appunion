...
应用互推联盟：开发者后台 + 开放 API + 运营审核。生产环境跑在 Render 上（Node + PostgreSQL + Persistent Disk）。Web 为 React + Vite。
...
一个 Web Service 托管前端静态资源和 API，数据在 PostgreSQL，图标在 Persistent Disk，定时任务跑在同一进程：

连上 Git 仓库后用 [render.yaml](render.yaml) 创建 Blueprint，或按 [DEPLOY.md](DEPLOY.md) 手动建服务。
