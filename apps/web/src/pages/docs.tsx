  ...
  return (
    <>
      ...
      <ol className="list-decimal space-y-2 pl-5 text-sm leading-7">
        ...
        <li>
          用 <code>Authorization: Bearer &lt;api_key&gt;</code> 调 <code>GET /v1/apps/recommend</code>，最多 10 条。
        </li>
        <li>卡片进入可视区域后再报曝光；用户点击后再报点击，然后打开商店或自定义跳转。</li>
...
export function DocsApi() {
  return (
    <>
      ...
      <p className="text-sm">Query：limit，默认 10，最大 10。同端、排除自己、仅推荐池，等权随机。</p>
      <h2 className="mt-6 font-medium">GET /v1/apps</h2>
      <p className="text-sm">全量已通过且未暂停的同端 App。page 从 1，page_size 默认 20，最大 50。</p>
  ...
  return (
    <>
      ...
      <ul className="list-disc space-y-2 pl-5 text-sm leading-7">
        <li>同端互推：Android 只出 Android，iOS、鸿蒙同理。</li>
...
export function DocsUi() {
  return (
    <>
      ...
      <p className="mt-4 text-sm text-muted">换一批 → GET /v1/apps/recommend；查看全部 → GET /v1/apps。</p>
    </>
  );
}
